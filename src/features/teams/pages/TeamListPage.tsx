...
import { deleteTeam, getAllTeams } from '@/services/teamsAction';
...
export const TeamListPage = () => {
  ...
  //   mutationFn: (data: Partial<Team>) => mockTeamApi.create(data),
  //   onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['teams'] }); toast.success('Team created successfully'); setFormOpen(false); },
  //   onError: () => toast.error('Failed to create team'),
  // });

  // const updateMutation = useMutation({
  //   mutationFn: ({ id, data }: { id: string; data: Partial<Team> }) => mockTeamApi.update(id, data),
  //   onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['teams'] }); toast.success('Team updated successfully'); setFormOpen(false); },
  //   onError: () => toast.error('Failed to update team'),
  // });

  // const deleteMutation = useMutation({
  //   mutationFn: (id: string) => mockTeamApi.delete(id),
  //   onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['teams'] }); toast.success('Team deleted successfully'); setDeleteOpen(false); },
  ...
  const handleCreate = () => { 
    setSelectedTeam(null);
     onOpen(); };
  const handleEdit = (team: Team) => { setSelectedTeam(team); setFormOpen(true); };
  const handleDelete = (team: Team) => { setSelectedTeam(team); setDeleteOpen(true); };
  const handleSubmit = (data: Partial<Team>) => {
    // if (selectedTeam) updateMutation.mutate({ id: selectedTeam.id, data });
  ...
  const {
    ...
    queryFn: () => getAllTeams(currentPage, pageSize),
    staleTime: 0,
