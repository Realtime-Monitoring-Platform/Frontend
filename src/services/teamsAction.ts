...
export const getAllTeamsByTeanntId = async (tenantId: string, page = 0, size = 10): Promise<Pagination<Team>> =>{
    try {//@GetMapping("/tenant/{tenantId}")
        const response = await api.get(`/query/teams/tenant/${tenantId}?page=${page}&size=${size}`);
